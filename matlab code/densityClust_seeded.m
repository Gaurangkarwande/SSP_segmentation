...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [numClust, clustInd, centInd, haloInd, delta] = densityClust_seeded(dist, seeds, dc, rho, isHalo, selectionMethod, minRho, minDelta)
...
%       haloInd: haloInd row vector [0 denotes no haloInd assignment]
    ...
    %[numClust, centInd] = decisionGraph(rho, delta, selectionMethod, minRho, minDelta);
    numClust = numel(seeds);
    ll = 1:numel(seeds);
    centInd = zeros(NE,1);
    centInd(seeds) = ll;
    if(centInd(ordRho(1)) == 0)
        centInd(ordRho(1)) = max(centInd);
        numClust = numClust + 1;
    end
