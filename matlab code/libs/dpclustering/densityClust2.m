function [numClust, clustInd, centInd, haloInd, delta] = densityClust2(dist, dc, rho, isHalo, centroid_id)
...
% Modified by D.U.P. from fileexchange/53922-densityclust
    ...
    indNearNeigh(centroid_id) = 0; % no point with higher density
    
    numClust = length(centroid_id);
    centInd = zeros(1, length(rho));
    cent_labels = 1:numClust;
    centInd(centroid_id) = cent_labels;
    
    %[numClust, centInd] = decisionGraph(rho, delta, selectionMethod, minRho, minDelta, num_topmost);
    ...
    clustInd = zeros(1, NE);
    disp(sum(indNearNeigh==inf));
    for i = 1 : NE
        if centInd(i) == 0 % not centroid
            %disp(indNearNeigh(ordRho(i)));
