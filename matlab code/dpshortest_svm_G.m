...
% peaks, to points. uses SVM                                              %
% Author: Diego Ulisse Pizzagalli                                         %
% Rev. 20190402-2311                                                      %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [centers, labels, path, dist] = dpshortest_svm_G(points, w, peaks, th_pruning, rho, SVMModel, W_SIZE)
%dpshortest assigns labels to points according with the shortest path from
%density peaks
%   points: array[num_pts x num_dim]. w: array[num_pts x num_pts] with the adjacency matrix. peaks: array[num_peaksx1] containing
%   the index of the centroids in points. th_pruning: scalar. 

%% Initialization and constants
    EPS_START = 0.1; %weight to connect the dummy node start to peaks.
    ...
    to = to(:);
    w = w(:);
    EPS_START = min(w(w>0))*EPS_START; % Forces it to be less than the minimum edge cost.
%% Graph pruning
    if(th_pruning > 0)
        from(w > th_pruning) = [];
        to(w > th_pruning)   = [];
...
%% Adding a dummy start node connected to peaks
    %TODO: some nodes may be unreachable.
...
%% Executing Dijkstra SSSP from start, through peaks to points
    ...
    A = (A+A');
    [dist, path, pred] = dijkstra_SSSP_svm(A, start_node_id, rho, SVMModel,W_SIZE);
%% Label assignment
